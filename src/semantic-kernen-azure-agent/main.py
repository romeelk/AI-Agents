...
def process_expenses(expense_agent: AzureAIAgent, prompt:str, expenses:str,project_client:AIProjectClient):
    # create a thread conversation
    thread: AzureAIAgentThread = AzureAIAgentThread(client=project_client)
    try:
        prompt_messages = [f"{prompt}:{expenses}"]
        # invoke the agent 
        agent_response =  expense_agent.get_response(thread_id=thread.id,messages=[prompt_messages])
        ...
        thread.delete()
        project_client.agents.delete_agent(expense_agent.id)
...
async def main():
    try:
        ...
        async with (
            ...
            process_expenses(expense_agent=sk_expenses_agent,prompt=user_prompt,project_client=client)
